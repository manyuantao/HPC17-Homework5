...
void compute_residual(double *u, double *rhs, double *res, int N, double h)
{
    ...
    for (i = 1; i < N; i++)
        for (j = 1; j < N; j++)
            res[id(i,j,N)] = rhs[id(i,j,N)] - (4 * u[id(i,j,N)] - u[id(i-1,j,N)] - u[id(i,j-1,N)] - u[id(i+1,j,N)] - u[id(i,j+1,N)]) / (h * h);
}
...
int main(int argc, char * argv[])
{
    ...
    for (iter = 0; iter < max_iters && res_norm/res0_norm > tol; iter++) {
        ...
        jacobi(u[levels-1], rhs[levels-1], N[levels-1], h[levels-1], 100);
